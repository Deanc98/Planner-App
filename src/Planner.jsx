...
function PlannerComponent() {
  ...
  const [showWeeklyModal, setShowWeeklyModal] = useState(false); // NEW STATE FOR WEEKLY OVERVIEW
  ...
  const navigateDate = useCallback((direction) => {
    ...
    setCurrentDate(newDate); // Allows navigating to any past or future date
  }, [currentDate]);
  ...
  const weeklyNotes = useMemo(() => {
    ...
    return weekData;
  }, [currentDate, currentDateKey]); // Recalculates whenever currentDate changes
...
// --- MAIN APPLICATION COMPONENT (The function name must match the export) ---
function PlannerComponent() {
  const [currentDate, setCurrentDate] = useState(START_DATE);
  const [notes, setNotes] = useState([]);
  const [newNoteText, setNewNoteText] = useState('');
  ...
  const navigateDate = useCallback((direction) => {
    ...
    // **PAST DATE FIX:** Allows navigation to any date in the past.
    // Only limits navigation to the defined future end date.
    if (newDate.getTime() > END_DATE.getTime() + MS_PER_DAY) {
      return; 
    }
  ...
  const handleDateChange = (event) => {
    ...
    // Limits selection to today and one year into the future (but allows past view)
    if (selectedDate.getTime() <= END_DATE.getTime() + MS_PER_DAY) {
        setCurrentDate(selectedDate);
    }
    setShowModal(false);
  };
...
// CRITICAL FIX: Exporting the component with its correct function name
export default PlannerComponent;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-sm">
            ...
            <input
              ...
              value={formatDateKey(currentDate)}
              onChange={handleDateChange}
              // Removed min attribute to allow selecting any date in the past
              max={formatDateKey(END_DATE)}
              aria-label="Select date"
            />
            <button
